...
/draft-plan
# Expected: 🔒 COMMAND LOCKED IN BOOTSTRAP MODE
...
**Checkpoint 1:** ✅ Context Gate blocks strategic ops until docs ready
...
**Checkpoint 2:** ✅ Router correctly classifies queries, planning works in EXECUTION mode
...
python -c "from mesh_server import open_db; conn = open_db(); conn.execute(\"UPDATE tasks SET status='blocked' WHERE id=1\"); conn.commit(); conn.close(); print('Task kicked back')"

# Verify status changed
python -c "from mesh_server import open_db; conn = open_db(); row = conn.execute('SELECT id, status, risk FROM tasks WHERE id=1').fetchone(); conn.close(); print(f'Task {row[0]}: status={row[1]}, risk={row[2]}')"
# Expected: status=blocked
```

**Checkpoint 3:** ✅ Kickback mechanism allows status updates
...
python -c "from mesh_server import open_db; conn = open_db(); conn.execute(\"UPDATE tasks SET status='reviewing' WHERE id=1\"); conn.commit(); conn.close(); print('Task in review')"
...
python -c "from mesh_server import submit_review_decision; r = submit_review_decision(1, 'APPROVE', 'Entropy Check: Passed ✅', 'HUMAN'); import json; print(json.loads(r))"
# Expected: status=OK or similar success

# Verify task completed
python -c "from mesh_server import open_db; conn = open_db(); row = conn.execute('SELECT status FROM tasks WHERE id=1').fetchone(); conn.close(); print(f'Status: {row[0]}')"
# Expected: status=completed
```

**Checkpoint 4:** ✅ Optimization Gate blocks approval without proof
...
python -c "from mesh_server import open_db; conn = open_db(); conn.execute(\"UPDATE tasks SET status='completed', files_changed='[\\\"migration.py\\\"]' WHERE id=2\"); conn.commit(); conn.close(); print('Task ready for QA')"

# Try to ship WITHOUT /verify (should block in control_panel.ps1)
# Simulate the /ship query:
python -c "from mesh_server import open_db; conn = open_db(); rows = conn.execute(\"SELECT id, desc, risk, qa_status FROM tasks WHERE risk='HIGH' AND qa_status!='PASS'\").fetchall(); conn.close(); print(f'HIGH risk tasks without PASS: {len(rows)}'); [print(f'  T-{r[0]}: {r[1][:50]} (qa_status={r[3]})') for r in rows]"
...
python -c "from mesh_server import open_db; conn = open_db(); row = conn.execute('SELECT qa_status FROM tasks WHERE id=2').fetchone(); conn.close(); print(f'QA Status: {row[0]}')"
# Expected: qa_status=PASS (if score ≥80) or WARN/FAIL

# Try to ship again (should succeed if PASS)
python -c "from mesh_server import open_db; conn = open_db(); rows = conn.execute(\"SELECT id FROM tasks WHERE risk='HIGH' AND qa_status!='PASS'\").fetchall(); conn.close(); print('Ship blocked!' if rows else 'Ship allowed ✅')"
# Expected: "Ship allowed ✅" (if verify passed)
```

**Checkpoint 5:** ✅ Risk Gate blocks HIGH risk tasks without QA PASS
...
# Temporarily rename tools/readiness.py → tools/readiness.py.bak
# Run strategic command - should fail open or show warning
# Restore tools/readiness.py
```

**Checkpoint 6:** ✅ Fail-open prevents deadlock (verified via smoke test)

...
class IntentExecutor:
    ...
    def execute(self, route: RouteResult) -> Dict[str, Any]:
        ...
        executors = {
            ...
            "NOOP": lambda a, p: {"status": "noop"}  # SAFETY-ALLOW: status-write
        }
    ...
    def _execute_resume(self, action: str, params: Dict) -> Dict:
        ...
        return {"status": "execute", "action": "trigger_continue"}  # SAFETY-ALLOW: status-write
    
    def _execute_queue_ops(self, action: str, params: Dict) -> Dict:
        """Modify the task queue."""
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        result = {"status": "success", "action": action}  # SAFETY-ALLOW: status-write
        ...
        try:
            if action == "add":
                ...
                cursor.execute(
                    "UPDATE tasks SET status='skipped' WHERE id=?",  # SAFETY-ALLOW: status-write
                    (params["task_id"],)
                )
                ...
                cursor.execute(
                    "UPDATE tasks SET status='pending', retry_count=0, auditor_status='pending', auditor_feedback='[]' WHERE id=?",  # SAFETY-ALLOW: status-write
                    (params["task_id"],)
                )
            ...
            result = {"status": "error", "error": str(e)}  # SAFETY-ALLOW: status-write
        finally:
            conn.close()
    ...
    def _execute_agent_direct(self, action: str, params: Dict) -> Dict:
        ...
        return {
            "status": "agent_call",  # SAFETY-ALLOW: status-write
            "agent": action,
    ...
    def _execute_query(self, action: str, params: Dict) -> Dict:
        ...
        return {"status": "query", "type": action, "params": params}  # SAFETY-ALLOW: status-write
    
    def _execute_hotkey(self, action: str, params: Dict) -> Dict:
        """Handle hotkey presses."""
        return {"status": "hotkey", "key": action, "params": params}  # SAFETY-ALLOW: status-write
    ...
    def _execute_exit(self, action: str, params: Dict) -> Dict:
        """Handle exit request."""
        return {"status": "exit"}  # SAFETY-ALLOW: status-write
    
    def _execute_chat(self, action: str, params: Dict) -> Dict:
        """Handle casual chat (no system change)."""
        return {"status": "chat", "text": params.get("text", "")}  # SAFETY-ALLOW: status-write
    
    def _execute_unknown(self, action: str, params: Dict) -> Dict:
        """Handle unknown intent."""
        return {"status": "unknown", "action": action}  # SAFETY-ALLOW: status-write
